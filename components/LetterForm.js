...
import { HiMail, HiUser, HiPhone, HiCalendar, HiArrowLeft, HiSave, HiCamera, HiCloudUpload, HiHashtag, HiOutlineFolder } from "react-icons/hi";
...
export default function LetterForm({ initialData, onSubmit, isSubmitting, backPath = "/admin", createdBy = "admin" }) {
  ...
  const renderTextarea = (label, name, placeholder = "", transliterate = false) => {
    const hasError = !!errors[name];
      ...
      <div className="flex items-center gap-4">
        ...
        <div>
          <h1 className="text-2xl md:text-3xl font-extrabold text-slate-800">
            {initialData ? "पत्र माहिती बदला (Edit Letter Details)" : "पत्र नोंदणी फॉर्म (Letter Registration Form)"}
          </h1>
          <p className="text-slate-500 text-sm mt-0.5">
            {initialData ? "नोंदणीकृत पत्राची माहिती अद्ययावत करा" : "आवक पत्र आणि त्याच्याशी संबंधित तपशीलांची नोंदणी करा"}
          </p>
        </div>
      </div>
      ...
      <form onSubmit={handleSubmit} className="space-y-6">
        ...
        <div className="bg-white rounded-3xl border border-orange-100 p-6 md:p-8 shadow-sm space-y-5">
          <div className="flex items-center gap-2.5 pb-2 border-b border-orange-50">
            ...
            <h2 className="text-base font-bold text-slate-800">१. पत्राची प्रतिमा / पुरावा (Letter Document)</h2>
          </div>
          {renderFileUpload("तपशील चा फोटो (Document Photo)", "photo")}
        </div>
        ...
        <div className="bg-white rounded-3xl border border-orange-100 p-6 md:p-8 shadow-sm space-y-5">
          <div className="flex items-center gap-2.5 pb-2 border-b border-orange-50">
            ...
            <h2 className="text-base font-bold text-slate-800">२. पत्राचा तपशील (Core Details)</h2>
          </div>
          <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
            {renderInput("विषय (Subject) *", "subject", "text", "उदा: रस्ता दुरुस्तीबाबत", true)}
            {renderInput("आवक क्रमांक (Inward Number) *", "inwardNumber", "text", "उदा: OUT/2026/102")}
          </div>
          <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
            {renderInput("पत्र कोणास संबोधित केले (Letter Addressed To)", "letterAddressedTo", "text", "उदा: मा. आयुक्त, महानगरपालिका", true)}
            {renderInput("खाते (Department / Account)", "department", "text", "उदा: बांधकाम विभाग", true)}
          </div>
          {renderTextarea("तपशील माहिती नोंदवा (Details / Description)", "details", "पत्राचे सविस्तर वर्णन किंवा टिप्पणी...", true)}
        </div>
        ...
        <div className="bg-white rounded-3xl border border-orange-100 p-6 md:p-8 shadow-sm space-y-5">
          <div className="flex items-center gap-2.5 pb-2 border-b border-orange-50">
            ...
            <h2 className="text-base font-bold text-slate-800">३. नियुक्ती आणि पुढील पाठपुरावा (Assignment & Follow-up)</h2>
          </div>
          <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
            {renderInput("नियुक्त केलेली व्यक्ती (Assigned Person)", "assignedPerson", "text", "उदा: राहुल देशमुख", true)}
            {renderInput("संपर्क क्रमांक (Contact Number)", "contactNumber", "tel", "उदा: 9876543210")}
          </div>
          <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
            {renderInput("पुढील कार्यवाही (Next Action / Status)", "nextAction", "text", "उदा: पाठपुरावा चालू आहे", true)}
            {renderInput("पुढील पाठपुराव्याची तारीख (Next Follow-up Date)", "followUpDate", "date")}
          </div>
        </div>
