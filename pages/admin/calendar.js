...
export default function AdminCalendar() {
  ...
  const getEventsForDate = (date) => {
    ...
    workers.forEach((w) => {
      ...
      if (w.DOB && isSameDayMonth(w.DOB, date)) {
        dayEvents.push({ type: "worker_birthday", label: `🎂 ${w.firstName} ${w.lastName} (वाढदिवस)`, name: `${w.firstName} ${w.lastName}` });
      }
      // Spouse Birthday
      if (w.maritalStatus === "विवाहित" && w.spouseName && w.spouseDOB && isSameDayMonth(w.spouseDOB, date)) {
        dayEvents.push({ type: "spouse_birthday", label: `🎂 ${w.spouseName} (${w.firstName} यांच्या पत्नी/पती वाढदिवस)`, name: w.spouseName });
      }
      // Wedding Anniversary
      if (w.maritalStatus === "विवाहित" && w.spouseName && w.anniversaryDate && isSameDayMonth(w.anniversaryDate, date)) {
        dayEvents.push({ type: "wedding_anniversary", label: `🎉 ${w.firstName} & ${w.spouseName} (लग्नाचा वाढदिवस)` });
      }
      // Father Birthday
      if (w.fatherName && w.fatherDOB && isSameDayMonth(w.fatherDOB, date)) {
        dayEvents.push({ type: "father_birthday", label: `🎂 ${w.fatherName} (${w.firstName} यांचे वडील वाढदिवस)` });
      }
      // Mother Birthday
      if (w.motherName && w.motherDOB && isSameDayMonth(w.motherDOB, date)) {
        dayEvents.push({ type: "mother_birthday", label: `🎂 ${w.motherName} (${w.firstName} यांची आई वाढदिवस)` });
      }
      // Parents Anniversary
      if (w.fatherName && w.motherName && w.parentsAnniversaryDate && isSameDayMonth(w.parentsAnniversaryDate, date)) {
        dayEvents.push({ type: "parents_anniversary", label: `🎉 ${w.fatherName} & ${w.motherName} (पालकांचा लग्नाचा वाढदिवस)` });
      }
    });
  ...
  const handleAddTodo = async (e) => {
    ...
    if (!newTodo.title.trim()) {
      toast.warning("कृपया कामाचे शीर्षक टाका.");
      return;
    }
    ...
    try {
      ...
      if (res.ok && data.success) {
        toast.success("काम यशस्वीरित्या जोडले!");
        ...
        toast.error("काम जोडण्यास अपयश आले.");
      }
    } catch {
  ...
  const handleToggleTodo = async (todo) => {
    try {
      ...
      const data = await res.json();
      if (res.ok && data.success) {
        toast.success("स्थिती अद्ययावत केली.");
        fetchData();
      }
    } catch {
      toast.error("अपडेट करताना चूक झाली.");
    }
  };
  ...
  const handleDeleteTodo = async (todoId) => {
    try {
      ...
      const data = await res.json();
      if (res.ok && data.success) {
        toast.success("काम यशस्वीरीत्या काढून टाकले.");
        fetchData();
      }
    } catch {
      toast.error("काम काढताना एरर आली.");
    }
  };
  ...
  const handleSaveEditTodo = async (e) => {
    ...
    try {
      ...
      toast.error("बदल सेव्ह करताना चूक झाली.");
    }
  };
  ...
  return (
    <>
      <Head>
        <title>प्रशासकीय दिनदर्शिका (Calendar) – Admin</title>
      ...
      <div className="p-4 md:p-8 space-y-6">
        ...
        <div className="flex flex-col sm:flex-row sm:items-center sm:justify-between gap-3">
          <div className="flex items-center gap-4">
            ...
            <div>
              <h1 className="text-2xl font-bold text-slate-800">प्रशासकीय दिनदर्शिका (Calendar Module)</h1>
              <p className="text-slate-500 text-sm mt-0.5">
                कार्यकर्त्यांचे वाढदिवस, लग्नाचे वाढदिवस आणि प्रशासकीय कामांचे व्यवस्थापन करा
              </p>
            </div>
          </div>
            ...
            <div className="space-y-6">
              ...
              <div className="flex justify-between items-start pb-4 border-b border-orange-50">
                <div>
                  <h2 className="text-xl font-extrabold text-slate-800">
                    {selectedDate.getDate()} {marathiMonths[selectedDate.getMonth()]} {selectedDate.getFullYear()}
                  </h2>
                  <p className="text-slate-400 text-xs mt-0.5">निवडलेल्या तारखेचे नियोजित कार्यक्रम</p>
                </div>
                <button
                  onClick={() => setSelectedDate(null)}
              ...
              <div className="space-y-3">
                <h3 className="text-xs font-bold text-slate-400 uppercase tracking-wider">
                  वाढदिवस आणि वर्धापनदिन (Birthdays & Anniversaries)
                </h3>
                {(() => {
                  ...
                  if (items.length === 0) {
                    return (
                      <p className="text-xs text-slate-400 bg-slate-50/50 border border-slate-100 p-3 rounded-xl">
                        या दिवशी कोणतेही वाढदिवस किंवा वर्धापनदिन नाहीत.
                      </p>
                    );
                  }
                  return (
              ...
              <div className="space-y-3">
                <div className="flex justify-between items-center">
                  ...
                  >
                    <HiPlus className="w-3.5 h-3.5" /> काम जोडा
                  </button>
                </div>
                ...
                {showAddForm && (
                  <form onSubmit={handleAddTodo} className="bg-orange-50/30 border border-orange-100/50 rounded-2xl p-4 space-y-3 animate-fade-in">
                    <h4 className="text-xs font-bold text-slate-800">नवीन काम जोडा</h4>
                    <div className="space-y-2.5">
                      <input
                        type="text"
                        placeholder="कामाचे शीर्षक..."
                        value={newTodo.title}
                ...
                {(() => {
                  ...
                  if (items.length === 0) {
                    return (
                      <p className="text-xs text-slate-400 bg-slate-50/50 border border-slate-100 p-3 rounded-xl">
                        या दिवसासाठी कोणतेही काम नियोजित नाही.
                      </p>
                    );
                  }
                  return (
